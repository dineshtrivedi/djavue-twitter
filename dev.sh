...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=ec2-34-254-191-164.eu-west-1.compute.amazonaws.com
...
workon djavue-twitter  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development djavue-twitter npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop djavue-twitter
    docker rm djavue-twitter
    docker run --name djavue-twitter -d --env-file /home/ubuntu/djavue-twitter.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/djavue-twitter:/dkdata \
        djavue-twitter start_web.sh
}
...
echo_green "Welcome to djavue-twitter's dev env"
